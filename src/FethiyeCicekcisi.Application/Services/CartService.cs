...
public class CartService
{
        ...
        string? userId, string? sessionId, int productId, int? variantId, int quantity = 1)
    {
        ...
        if (variantId.HasValue)
        {
            ...
            if (variant is null) return (false, "Seçilen seçenek bulunamadı.");
            if (!variant.IsActive) return (false, "Bu seçenek aktif değil.");
        ...
        else
        {
            ...
            // ya da seed manifest'indeki secenekler/stok satırıyla varyant olarak girilir.
