...
namespace FethiyeCicekcisi.Infrastructure.Repositories;
...
public class ProductRepository : Repository<Product>, IProductRepository
{
        ...
        int? occasionId = null)
    {
        ...
        if (occasionId.HasValue)
            query = query.Where(p => p.ProductOccasions.Any(po => po.OccasionId == occasionId.Value));

        if (!string.IsNullOrWhiteSpace(size))
        {
            // Seçenekler admin panelinde serbest metin girildiği için ("11 Adet" / "11 adet")
            // harf duyarsız karşılaştırılır. Stoğu bitmiş seçenek "var" sayılmaz.
        ...
        {
            // Postgres'te LIKE (EF'in Contains çevirisi) harf duyarlıdır — "orkide" yazan
            // müşteri "Orkide"yi bulamazdı. ILIKE ile harf duyarsız arama yapılır.
...
public class OccasionRepository : Repository<Occasion>, IOccasionRepository
{
    ...
    public async Task<Occasion?> GetBySlugAsync(string slug) =>
        await _dbSet.FirstOrDefaultAsync(o => o.Slug == slug && !o.IsDeleted);

    public async Task<IEnumerable<Occasion>> GetActiveAsync() =>
        await _dbSet.Where(o => o.IsActive && !o.IsDeleted)
                    .OrderBy(o => o.DisplayOrder)
                    .ToListAsync();
}

public class DeliveryZoneRepository : Repository<DeliveryZone>, IDeliveryZoneRepository
{
    public DeliveryZoneRepository(AppDbContext context) : base(context) { }

    public async Task<IEnumerable<DeliveryZone>> GetActiveAsync() =>
        await _dbSet.Where(z => z.IsActive && !z.IsDeleted)
...
public class PromoCodeRepository : Repository<PromoCode>, IPromoCodeRepository
{
    ...
    public async Task<PromoCode?> GetByCodeAsync(string code) =>
        await _dbSet.FirstOrDefaultAsync(p => p.Code == code.Trim().ToUpper() && p.IsActive && !p.IsDeleted);
}
